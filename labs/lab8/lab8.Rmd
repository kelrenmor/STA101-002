...
download.file("https://kelrenmor.github.io/STA101-002/labs/lab8/sta101_lab8.Rmd",
              destfile="sta101_lab8.Rmd")
```
</div>
...
are viewed to be better looking receive higher instructional ratings. In this lab we will analyze the data from this study in order to learn
what goes into a positive professor evaluation. 

(Daniel S. Hamermesh, Amy Parker, Beauty in the classroom: instructors pulchritude and 
...
4.  Replot the scatterplot, but this time add the argument `position=position_jitter()` to the
    `geom_point()`. (Tye `?position_jitter` in the Console to learn more.) What was
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    with `+ geom_smooth(method='lm', se=FALSE)`. Write out the equation for the linear model and
...
# Plot relationship between one students' individual ratings and beauty score
ggplot(evals, aes(x=bty_f1lower, y=bty_avg)) + geom_point()
# Get correlation between one students' individual ratings and beauty score
...
the gender term into the model by simply typing `+ gender` after the tilde in the function.
...
11. Which variable would you expect to have the highest p-value in this
    ...
    have any association with the professor score. (Note you don't have
    to be perfectly correct, just give your honest guess.)
