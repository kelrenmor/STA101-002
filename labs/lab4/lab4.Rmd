...
We will also use the **infer** package for resampling, and the **ggplot2** package for visualizations.
...
download.file("https://kelrenmor.github.io/STA101-002/labs/lab4/sta101_lab4.Rmd",
              destfile="sta101_lab4.Rmd")
```
...
```{r bar-plot-pop, fig.height=2.5, fig.width=10, fig.show='asis'}
...
Here, we use R to take 15,000 different samples of size 50 from the population, calculate the number and proportion of *Doesn't benefit* responses in each sample, and store each result in a tibble called `sample_props50`. 
...
sample_props50 <- global_monitor %>%
  ...
  summarise(n = sum(scientist_work == "Doesn't benefit"),
...
1.  Use the app shown at https://openintro.shinyapps.io/sampling_distributions/ (in the same place is in this document, under Exercise 6) to create sampling distributions of proportions of
