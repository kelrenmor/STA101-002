...
download.file("https://kelrenmor.github.io/STA101-002/labs/lab7/sta101_lab7.Rmd",
              destfile="sta101_lab7.Rmd")
```
</div>
...
examining the linear relationship between runs scored in a season and a handful 
...
us to predict a team's runs scored in a season.
...
we'll consider one of the seven traditional variables. At the end of the lab, you'll 
work with the other variables on your own.
...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    variable `at_bats` as the explanatory variable. Does the relationship look linear?
    If you knew a team's `at_bats`, would you be comfortable using a linear model 
...
<div id="boxedtext">
In this section you will use an interactive function to investigate what we mean by "sum of squared residuals". You will need to run this function in your console, not in your 
markdown document. Running the function also requires that the `mlb11` dataset is loaded in your environment.
</div>
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    squares that you got? What if you intentionally choose a poorly fitting line -- 
    how much larger does the sum of squares get?
...
What does the slope tell us in the context of the relationship between success of 
a team and its at-bats? For a one unit increase in at-bats we would expect runs to 
increase on average by 0.6305.

What does the intercept tell us in the context of the relationship between success of 
a team and its at-bats? If at-bats are zero, we would expect runs to be -2789.2429
on average. But what is a negative run!? Does this make sense? Consider the values
taken by the at-bat variable -- we only observe data within the range of 5400 to 5750
at-bats, so we never get anywhere near 0 at-bats. In this case, the intercept doesn't
provide us with much meaningful information.
...
Consider the Philadelphia Phillies, which have 5,579 at-bats.
...
4.  If a team manager saw the least squares regression line and not the actual 
    ...
    Is this an overestimate or an underestimate for the Philadelphia Phillies, 
    and by how much? In other words, what is the residual for this prediction?
...
Note that the syntax for making a normal probability plot is a bit different than what you're used to seeing with `ggplot`: we set `sample` equal to the residuals instead of `x`, and we set a statistical method `qq`, which stands for "quantile-quantile", another name commonly used for normal probability plots.
...
7.  Based on the residuals vs. fitted plot, does the constant variability condition 
    appear to be met?
...
8.  Choose another one of the seven traditional variables from `mlb11` besides 
    `at_bats` (i.e., one of hits, home runs, batting average, strikeouts, stolen bases, 
    and wins) that you think might be a good predictor of `runs`. Produce a 
    scatterplot of the two variables and fit a linear model. At a glance, does 
    there seem to be a linear relationship?

9. How does this relationship compare to the relationship between `runs` and 
...
10. Check the model diagnostics for the regression model using the 
    three conditions we assessed above.

11. *Bonus problem (optional) -- if you do this correctly, you will be forgiven for 
    up to 5 missed points on the remainder of the assignment, taking your total up to a 
    max of 100 points.* Now that you can summarize the linear relationship between 
    two variables, 
    investigate the relationships between `runs` and each of the other five 
    traditional variables, and also `runs` and each of the three newer variables
    (i.e. the statistics used by the author of *Moneyball*). 
    Which variable seems to be the best predictor of `runs`?
     (Hint: figure out which has the highest R$^2$ by quickly running `lm()` 
     using each possible explanatory variable and jotting down each of 
     the resulting R$^2$ values.)
    Support your conclusion by stating the R$^2$ value for this model. 
    Using the limited (or not so limited) information you know about 
    these baseball statistics, does your result make sense?
    You don't have to show all of your work or include all of the code you use -- 
    feel free to just run the models in your console.
