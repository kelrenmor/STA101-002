...
The `inference` function has a `method="simulation"` argument that allows you to perform bootstrapping rather than getting theoretical results. That is, if conditions for using the CLT are violated (non-normal and small sample size) you can still perform inference.
...
Because the data are a random sample of 1,000 cases from births recorded in NC, independence within and between groups is satisfied. However, as we saw in our box plots, there are more outliers than we would expect from normally distributed data. However, because $n>30$ for both groups the conditions for the CLT are met and we could proceed with theoretically based inference.

We show inference using simulation-based methods (the bootstrap) for illustrative purposes, although in this case conditions for the theoretical test have been met. Note that to use CLT-based methods you would just replace `method="simulation"` with `method="theoretical"`.
